"""
Insurance Journal scraper implementation.

Scrapes articles from Insurance Journal using Apify Web Scraper actor.
"""
...
class InsuranceJournalSource(NewsSource):
    ...
    - Article containers: article, .article-item, .news-item
    - Title: h2 a, h3 a, .article-title a
    - Description: .article-excerpt, .summary, p
    ...
    def scrape(self) -> List[Dict[str, Any]]:
        """
        Scrape latest articles from Insurance Journal.

        Returns:
            List of article dictionaries matching standard schema.
            Empty list if scraping fails.
        """
        log = logger.bind(source="insurance_journal", url=self.source_url)
            ...
            run_input = {
                ...
                "pageFunction": """
                    async function pageFunction(context) {
                        ...
                        $('article, .article-item, .news-item').each((index, element) => {
                            ...
                            const description = $article.find('.article-excerpt, .summary, p').first().text().trim();
    ...
    def _normalize_article(self, raw_article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_name": raw_article.get("source_name", "Insurance Journal")
        }
