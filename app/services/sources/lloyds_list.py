"""
Lloyd's List scraper implementation.

Scrapes articles from Lloyd's List using Apify Web Scraper actor.
"""
...
class LloydsListSource(NewsSource):
    """
    Scraper for Lloyd's List website.

    Uses Apify's web-scraper actor to extract article data from structured HTML.

    CSS Selectors (with fallbacks):
    - Article containers: article, .article-card, .story-card
    - Title: h2 a, h3 a, .article-title a
    - Description: .article-summary, .teaser, p
    ...
    def scrape(self) -> List[Dict[str, Any]]:
        """
        Scrape latest articles from Lloyd's List.

        Returns:
            List of article dictionaries matching standard schema.
            Empty list if scraping fails.
        """
        log = logger.bind(source="lloyds_list", url=self.source_url)
        ...
        try:
            ...
            run_input = {
                ...
                "linkSelector": "article a[href], h2 a[href], h3 a[href], .article-title a[href]",
                "pageFunction": """
                    async function pageFunction(context) {
                        ...
                        $('article, .article-card, .story-card').each((index, element) => {
                            ...
                            const title = $article.find('h2 a, h3 a, .article-title a, h2, h3, .article-title').first().text().trim();
                            if (!title) return; // Skip if no title

                            // Extract description with fallbacks
                            const description = $article.find('.article-summary, .teaser, p').first().text().trim();
    ...
    def _normalize_article(self, raw_article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_name": raw_article.get("source_name", "Lloyd's List")
        }
