"""
Business Insurance scraper implementation.

Scrapes articles from Business Insurance using Apify Web Scraper actor.
"""
...
class BusinessInsuranceSource(NewsSource):
    """
    Scraper for Business Insurance website.

    Uses Apify's web-scraper actor to extract article data from structured HTML.

    CSS Selectors (with fallbacks):
    - Article containers: article, .article-card, .story
    - Title: h2 a, h3 a, .headline a
    - Description: .teaser, .deck, .excerpt, p
    - URL: article a[href], h2 a[href], h3 a[href]
    - Date: time, .published, .entry-date
    """

    def scrape(self) -> List[Dict[str, Any]]:
        """
        Scrape latest articles from Business Insurance.

        Returns:
            List of article dictionaries matching standard schema.
            Empty list if scraping fails.
        """
        log = logger.bind(source="business_insurance", url=self.source_url)
        ...
        try:
            ...
            run_input = {
                ...
                "linkSelector": "article a[href], h2 a[href], h3 a[href], .headline a[href]",
                "pageFunction": """
                    async function pageFunction(context) {
                        ...
                        $('article, .article-card, .story').each((index, element) => {
                            ...
                            const title = $article.find('h2 a, h3 a, .headline a, h2, h3, .headline').first().text().trim();
                            if (!title) return; // Skip if no title

                            // Extract description with fallbacks
                            const description = $article.find('.teaser, .deck, .excerpt, p').first().text().trim();
    ...
    def _normalize_article(self, raw_article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_name": raw_article.get("source_name", "Business Insurance")
        }
