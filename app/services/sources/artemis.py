"""
Artemis scraper implementation.

Scrapes articles from Artemis using Apify Web Scraper actor.
"""
...
class ArtemisSource(NewsSource):
    """
    Scraper for Artemis website.
    ...
    def scrape(self) -> List[Dict[str, Any]]:
        """
        Scrape latest articles from Artemis.

        Returns:
            List of article dictionaries matching standard schema.
            Empty list if scraping fails.
        """
        log = logger.bind(source="artemis", url=self.source_url)
        ...
        try:
            ...
            run_input = {
                ...
                "linkSelector": "article a[href], h2 a[href], h3 a[href], .entry-title a[href]",
                "pageFunction": """
                    async function pageFunction(context) {
                        ...
                        $('article, .post, .entry').each((index, element) => {
                            ...
                            const title = $article.find('h2 a, h3 a, .entry-title a, h2, h3, .entry-title').first().text().trim();
    ...
    def _normalize_article(self, raw_article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_name": raw_article.get("source_name", "Artemis")
        }
